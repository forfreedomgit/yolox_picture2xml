    ...
    def __call__(self, target):
        ...
        for obj in target.iter("object"):
            ...
            if not self.keep_difficult and difficult:
                continue
            name = obj.find("name").text.strip() # 正常VOC数据
#             name = obj.find("label").text.strip() # 结核数据集
...
#         width = 1632   # 结核数据集特殊，没有size，也就没有wigth height
#         height = 1224
...
#         imagesetfile = os.path.join(rootpath, "ImageSets", "Main", name + ".txt")
        imagesetfile = os.path.join(rootpath, "ImageSets", "Main", 'val' + ".txt")
